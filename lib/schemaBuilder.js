...
async function makeBaseSchema(options) {
    ...
    await fs.promises.writeFile(`${folder}base.graphql`, baseContent)
        .catch(console.error)
...
async function filteredTables(options) {
    ...
    // excluding tables
    tables = tables.filter(table => options.excludeTables.includes(table) === false);
...
async function writeToFile(directory, file, content) {
    ...
    await fs.promises.writeFile(`${directory}${file}.graphql`, content)
        .catch(console.error)
...
function generateSqlQuery(gql, options, queriesName, queryName, modelName) {
    ...
    const middleware = runMiddleware(modelName, generatedSqlKey, generatedSqlValue, 'query', options);
...
${comment}`
        ...
        const overwrite = runOverwrite(modelName, generatedSqlKey, generatedSqlValue, options);
        generatedSqlKey = overwrite.key;
        generatedSqlValue = overwrite.value;
        const middleware = runMiddleware(modelName, generatedSqlKey, generatedSqlValue, 'column', options);
        generatedSqlKey = middleware.key;
        generatedSqlValue = middleware.value;
        ...
        if (type === 'query') {
            if (middlewares.queries) {
                if (middlewares.queries.middleware) {
                    value += ` @middleware(checks: ${JSON.stringify(middlewares.queries.middleware)})`;
                }
                if (middlewares.queries.permission) {
                    ...
                    if (middlewares.queries.permission.hasOwnProperty('any')) {
                        permissionString += permissionString !== '' ? ' ' : '';
                        permissionString += `any: ${JSON.stringify(middlewares.queries.permission.any)}`;
                    }
            ...
            middlewares.columns.forEach(middleware => {
                if (key === middleware.key) {
                    if (middleware.hasOwnProperty('middleware')) {
                        value += ` @middleware(checks: ${JSON.stringify(middleware.middleware)})`;
                    }
                    let permissionString = '';
...
function makeSingular(options, str) {
    if (options.singularExclude && options.singularExclude.includes(str.toLowerCase())) {
        return str;
    }
...
async function getDatabaseForeigns(tableName) {
    ...
    return new Promise((resolve, reject) => {
        connection.query(`SELECT CONSTRAINT_SCHEMA, REFERENCED_TABLE_NAME as foreign_table FROM information_schema.REFERENTIAL_CONSTRAINTS WHERE TABLE_NAME = '${tableName}' AND CONSTRAINT_SCHEMA = '${connectionOptions.database}' GROUP BY foreign_table`, function (error, results, fields) {
            if (error) {
                connection.end();
                return reject(error);
            }
